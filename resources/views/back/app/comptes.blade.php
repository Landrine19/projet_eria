@extends('app.template.base',["title" => "Les comptes"])
      ...
      <table id="example1" class="table table-bordered table-striped">
            <thead class="thead-dark">
                ...
                <tr>
                    ...
                    <th class="text-center">A recouvrer/ Restant </th>
                    <th></th>
                </tr>
                </tr>
            </thead>
            <tbody class="text-center">
                @foreach($data->donnees as $compte)
                    <tr>
                        <td>{{$compte->nom ." ".$compte->prenoms }}</td>
                        <td>{{$compte->email}}</td>
                        <td>{{$compte->localisation}}</td>
                        <td>{{$compte->contact}}</td>
                        <td>{{$compte->sar ."/". $compte->sdr}}</td>
                        <td>
                            <button  title="Supprimer" id="{{$compte->id}}" class="btn btn-danger btn-sm delete" data-toggle="modal" data-target="#delete">
                                <i class="fas fa-trash"></i>
                            </button>
                            <button title="Modifier" id="{{$compte->id}}" data-toggle="modal" data-target="#form" class="edit btn btn-sm bg-info">
                                <i class="fas fa-pen"></i>
                            </button>
                            <a href="{{route('comptes.space',$compte->id)}}" class="btn btn-sm btn-primary">
                                <i class="fas fa-user"></i> Espace compte
                            </a>
                        </td>
                    </tr>
                @endforeach
...
<form method="post" id="add-form" enctype="multipart/form-data">
    <div class="modal-header flex-column text-center bg-primary">
        <h4 class="modal-title w-100">Ajouter un compte</h4>
    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            ...
            <select name="user_id" id="user_id"  class="form-control">
                <option value="">Selectionnez un agent suiveur pour ce compte</option>
